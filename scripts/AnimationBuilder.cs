...
public partial class AnimationBuilder : Node
{
    ...
    public void Build()
    {
        ...
        if (animPlayer == null)
        {
            GD.PrintErr("[AnimBuilder] No AnimationPlayer found in Aria subtree");
            return;
        }
        ...
        {
            if (BakeRetargeted(kvp.Key, kvp.Value, tgtSkel, lib)) added++;
            else skipped++;
        }
    ...
    /// </summary>
    private bool BakeRetargeted(string shortName, string fbxPath, Skeleton3D tgtSkel, AnimationLibrary lib)
    {
        ...
        try
        {
            ...
            foreach (var bt in boneTracks)
            {
                ...
                anim.TrackSetPath(tr, $"Armature/Skeleton3D:{bt.Name}");
