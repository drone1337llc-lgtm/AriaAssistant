...
public partial class ExpressionController : Node
{
    ...
    [Export] public float BlinkMinInterval = 2.5f;
    [Export] public float BlinkMaxInterval = 6.0f;
    [Export] public float BlinkDuration = 0.16f;
    ...
    public void Setup(Node ariaRoot)
    {
        ...
        _slotEyeJoy       = Resolve("Fcl_EYE_Happy");
        _slotBrwFun       = Resolve("Fcl_BRW_Fun", "Fcl_BRW_Happy");
        _slotEyeFun       = Resolve("Fcl_EYE_Happy");
    ...
    public override void _Process(double delta)
    {
        ...
        float blinkVal = 0f;
        _blinkTimer += d;
        if (_blinkPhase < 0f && _blinkTimer >= _nextBlink) _blinkPhase = 0f;
        if (_blinkPhase >= 0f)
        {
            _blinkPhase += d / Math.Max(0.02f, BlinkDuration);
            blinkVal = _blinkPhase < 0.5f ? _blinkPhase * 2f : 1f - (_blinkPhase - 0.5f) * 2f;
