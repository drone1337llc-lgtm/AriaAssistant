...
public partial class HealthMonitor : Node
{
    ...
    private string ResolvePath()
    {
        ...
        {
            // The original code silently fell through. That meant: if the user's
            // log path was unreachable (e.g. permission denied, drive offline),
            // Aria just quietly wrote to user:// and the operator never knew.
            // Now we surface it; the fall-through still happens so the run
            // continues, but with a breadcrumb in the Godot Output panel.
            GD.PrintErr($"[Health] could not resolve log path '{HealthLogPath}': {e.Message} — falling back to user://");
        }
