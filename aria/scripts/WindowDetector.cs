...
public partial class WindowDetector : Node
{
	...
	// Throttle for the ForegroundTitle poll-error log. We don't want to spam
	// the Output panel every 2s if Win32 is failing repeatedly; once per 30s is
	// enough to surface a stuck poll without flooding the breadcrumb stream.
	private DateTime? _lastTitlePollError = null;
	...
	private void RefreshForegroundTitle()
	{
		...
		{
			// Original code silently kept the last known title. That's fine for
			// transient Win32 errors, but if the call is failing every poll
			// the operator has no signal that screen-watching is broken. Log
			// at low frequency to avoid spamming the Output panel.
			_lastTitlePollError ??= DateTime.UtcNow;
			if ((DateTime.UtcNow - _lastTitlePollError.Value).TotalSeconds > 30)
			{
				GD.PrintErr($"[WindowDetector] ForegroundTitle poll failed: {e.Message} (throttled to once per 30s)");
				_lastTitlePollError = DateTime.UtcNow;
			}
		}
	}
